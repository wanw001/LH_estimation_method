...
sizeProp.acr <- function(linf, mk, l50, l95r, l0r, sa, cv, w, mr, mo){
  ...
  xxo <- NULL
  for (i in 0:amax){
    xo <- i*fr*12 + ((mo-mr[length(mr)])*fr):((mo-mr[1]+1)*fr-1)
    xxo <- c(xxo, xo)
  }
  xxo <- xxo[xxo>=0 & xxo<=(X-1)]
  md <- -log(sa)/(X-1)
  a0 <- 1/k*log(1-l0r)
  Ax <- xxo/(X-1)*amax
  ...
  L <- seq(lmin+w/2, lmax-w/2, by=w)
  Lu <- seq(lmin+w, lmax, by=w)
  
  ## Costruct age composition
  R <- 1
  Surv <- 1
  for (x in 2:X){
  ...
  ## Convert relative abundance at age to proportion of fish at size bin
  PP <- matrix(rep(NA, nl*nxo), nrow=nl, ncol=nxo)
  for (x in 1:nxo){
    la <- Lx[x]
    sd <- cv*la
    lu <- lmin + w
  ...
  Nl0 <- unlist((matrix(Nxo, nrow=1)%*%t(PP))[1,])
  pl <- Nl0/sum(Nl0)
  
  ## Computing probability of observed abundance at size bin
  l95 <- l95r*l50
  alp <- log(19)/(l95-l50)
